 ...
 ** is the exponentiation function (xn)
 ...
 Es = saturated vapour pressure at Avtemp) Note that these values are not exactly the same as for SVAP.
 avt = AVTEMP + 237.3
 Es = 6.1078 * exp((17.269 * AVTEMP) / avt)
 ...
 The calculation of EA, HMM, EVAPG and EVAPW are described in detail in "The Evaluation of Penman's Natural Evaporation Formula by Electronic Computer" G. Berry, Aust. J. Appl. Sci, 1964, 15: 61-64.
...
function get($head)
{
	...
	for ($col = 0; $col < $nbCols; $col++) {
		...
		for ($row = 0; $row < $nbRows; $row++) {
			...
			if ($data->val($row,$col,1)=== $head)
			{
				...
				$coordDAY = $row.",".$col;
				//echo $data->val($row,$col,1)." = " . $coordDAY."<br />";
	...
	{
		$header[$i] = $data->val($row, $coordDAYS[1],1);
...
<form
	...
	<input
		...
		type="submit"
		value="Submit"
		onclick="document.formImport.vFileName.value = ''" />
	...
	<table
		...
		<tr id="Headers">
			...
			<th id="psmd">SN2D</th>
			<th id="dda">SNDECL</th>
			<th id="dda">SNDECL2</th>
			<th id="ddb">CSDECL</th>
			<th id="ddb">CSDECL2</th>
			<th id="dda">CSL</th>
			<th id="ddb">SNL</th>
			<th id="EVAP_G">CSHASS</th>
			<th id="EVAP_W">SNHASS</th>
			<th id="EVAP_W">HASS</th>
			<th>hrday</th>
			<th>ea_grass</th>
			<th>ea_water</th>
			<th>avt</th>
		...
		<?php
		$accdda = 0;$accddb = 0;$psmd[0] = 0;
		for ($row = 1; $row<count($day); $row++)
		{ if ($row >1){
			$dd = get_dds($row);
		}
		array_walk($dd, 'formatResults');
		...
		if ($date) {
			...
			{
				$psmd[$row] = $psmd[$prev] + $dd['evapg'] - $rain[$row];
			}
		}
